...
// ==/UserScript==
(function () {
	var TAAT_main = function () {
		console.log('Tunnel Info loaded');
		function CreateTAAT() {
			qx.Class.define("TAAT", {
				type : "singleton",
		...
		function TAAT_checkIfLoaded() {
			try {
				if (typeof qx !== 'undefined' && typeof qx.locale !== 'undefined' && typeof qx.locale.Manager !== 'undefined' && qx.core.Init.getApplication() && ClientLib.Data.MainData.GetInstance().get_Player().get_Faction() !== null) {
					CreateTAAT();
					window.TAAT.getInstance().initialize();
				} else {
					window.setTimeout(TAAT_checkIfLoaded, 1000);
				}
			} catch (e) {
				console.log("TAAT_checkIfLoaded: ", e);
			}
		}
	...
	try {
		var TAAT = document.createElement("script");
		TAAT.innerHTML = "(" + TAAT_main.toString() + ")();";
		TAAT.type = "text/javascript";
		if (/commandandconquer\.com/i.test(document.domain)) {
			document.getElementsByTagName("head")[0].appendChild(TAAT);
		}
	} catch (e) {
		console.log("TAAT: init error: ", e);
	}
})();