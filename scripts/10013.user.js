				...
				qx.Class.define("FlunikTools.Main", {
					...
					members: {
						...
						initialize: function() {
							...
							BuildingsButton = new qx.ui.form.Button("Gebaude AUS", null).set({
								toolTipText: "nur Gebaude upgraden",
								width: 110,
								height: 30,
								maxWidth: 110,
								maxHeight: 30,
							...
							CommandBuildingChoice = new qx.ui.form.Button("Nur Ressis AUS", null).set({
								toolTipText: "Ermöglicht, nur Resourcen upzugraden,Hauptgebaude wie Bauhof, Vz, Ve usw werden ausgelassen",
								width: 110,
								height: 30,
								maxWidth: 110,
								maxHeight: 30,
							...
							_this.FehlerWindow.addListener("close", function(e) {
								...
								if (window.FlunikTools.Main.getInstance().autoUpdateHandleBuilding != null) {
									...
									BuildingsButton.setLabel("Gebaude AUS");
							...
							BuildingsButton.addListener("click", function (e) {
								...
								if (window.FlunikTools.Main.getInstance().autoUpdateHandleBuilding != null) {
									...
									BuildingsButton.setLabel("Gebaude AN");
						...
						autoUpgradePopupOeffnen : function(){
							...
							var tabviewZWEI = new qx.ui.tabview.Page("Info", null);
							tabviewZWEI.setLayout(new qx.ui.layout.VBox());
							...
							for (basis in allebasen){
								...
								var allegebaude = allebasen[basis].get_Buildings().d;
								...
								basisEinheiten['GebAktiv'] = (new Array(new qx.ui.form.CheckBox("<b><u>Gebaude Aktiv</b></u>").set({value: wert, enabled: status, rich: true}), 99));
								...
								//page.add(new qx.ui.basic.Label('<b><u>Gebaude:</b></u>').set({rich: true}),{row: 2, column: 0});
								
								for(gebaude in allegebaude){
									var schondrin = false;
									var gebaudename = allegebaude[gebaude].get_TechGameData_Obj().dn;
									var techname = allegebaude[gebaude].get_TechName();
									var gebaudeId = allegebaude[gebaude].get_Id();
									var istSammler = techname == ClientLib.Base.ETechName.Harvester ? true : false;
									var ressityp = allegebaude[gebaude].get_CollectResourceType();
									var objekttyp = 'geb';
									_this.objektcount++;
									
									if(gebaudeliste.length == 0){
										//gebaudeliste.push(gebaudename + ' (' + ressityp + ':' + techname + ')');
										gebaudeliste.push(new Array(gebaudename, ressityp, techname, objekttyp));
									}
									
									for(var i = 0; i < gebaudeliste.length; i++){
										//if(gebaudeliste[i] == (gebaudename + ' (' + ressityp + ':' + techname + ')')){
										if((gebaudeliste[i][0] == gebaudename) && (gebaudeliste[i][1] == ressityp) && (gebaudeliste[i][2] == techname)){
											schondrin = true;
											break;
										}
									}
								...
								for(elem in gebaudeliste){
										var eintrag = gebaudeliste[elem][0];
										var wert = true;
										var status = true;
										var sammler = ClientLib.Base.Tech.GetTechDisplayNameFromTechId(ClientLib.Base.ETech.FOR_Harvester);
										if ((gebaudeliste[elem][2] == ClientLib.Base.ETechName.Harvester) && (gebaudeliste[elem][1] == ClientLib.Base.EResourceType.Crystal)){
											eintrag = sammler + ' Kristall';
										}
										if((gebaudeliste[elem][2] == ClientLib.Base.ETechName.Harvester) && (gebaudeliste[elem][1] == ClientLib.Base.EResourceType.Tiberium)){
										...
										basisEinheiten['geb'][eintrag] = (new Array(new qx.ui.form.CheckBox(eintrag).set({value: wert, enabled: status}), gebaudeliste[elem][1], gebaudeliste[elem][2]));
							...
							var boxF = new qx.ui.container.Composite(new qx.ui.layout.VBox(1)).set({padding: 0, decorator: "pane-light-opaque"});//enhalt Tabview
							boxF.add(_this.tabview);
							
							var boxE = new qx.ui.container.Composite(new qx.ui.layout.HBox(5)).set({margin: 0, padding: 10, decorator: "pane-light-opaque"});//enhalt liste + statusbox
							...
							var lblGa = new qx.ui.basic.Label("Ranganderung seit Start: " + rangAktuell);//.set({ alignX: "center", alignY: "center"});
							...
							if(_this.timer == true){
								timer.start(function(userData, timerId)
								{
									...
									lblGa.setValue("Ranganderung seit Start: " + rangAktuell);
                        ...
                        EinAusLabel : function (sender) {
							...
							ttt = _this.geb > 0 ? ttt + "<br>Gebaude <b>EIN</b>" : ttt;
						...
						DarfUpgraden : function(kat, basis, gebaude, ressityp, einheit){
							try{
								var _this = window.FlunikTools.Main.getInstance();
								...
								var ugo = einheit == null ? gebaude : einheit;
								var sammler = ClientLib.Base.Tech.GetTechDisplayNameFromTechId(ClientLib.Base.ETech.FOR_Harvester);
								...
								console.info('BASISLAYOUT GEaNDERT! LADE BASISDATEN ERNEUT...');
						...
						canUpgradeBuilding: function (building, city) {
							...
							var gebaudename = building.get_TechGameData_Obj().dn;
							var basisname = city.get_Name();
							
							if(!_this.DarfUpgraden('geb', basisname, gebaudename, ressityp, null)) {
